...
public:
    DeviceStateCache(const AppConfig& config, DeviceConnectionManager& connections, const AlarmStore& alarms);
    ~DeviceStateCache();
...
private:
    ...
    void worker_loop();
    void refresh_configured_device(const DeviceConfig& device);
